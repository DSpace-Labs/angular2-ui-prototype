                ...
                <div *ngIf="context.limit < context.total" class="form-inline">
                    <ul class="pagination">
                        ...
                        <li *ngIf="context.page > 1">
                            <!-- Router Link -->
                            <a [routerLink]="[path, {id: context.id, page: previous}]" (click)="pageOnDashboard(context.id, previous)">
                        ...
                        <li *ngFor="#i of pages" [ngClass]="{active: i == context.page}" [class.disabled]="i == '...'">
                            <!-- Router Link -->
                            <a [routerLink]="[path, {id: context.id, page: i}]" *ngIf="i != '...'" (click)="pageOnDashboard(context.id, i)">{{ i }}</a>
                            <span *ngIf="i == '...'">{{ i }}</span>
                        </li>

                        <li *ngIf="context.page < context.pageCount">
                            <!-- Router Link -->
                            <a [routerLink]="[path, {id: context.id, page: next}]" (click)="pageOnDashboard(context.id, next)">
...
export class PaginationComponent {
    ...
    /**
     * A String that represents the path of the current component.
     */
    path: String;
    ...
    ngOnInit() {
        ...
        this.path = this.router.hostComponent.name == "DashboardComponent" ? "/" : "/" + this.context.component;
    }
